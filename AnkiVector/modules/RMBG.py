...
async def _(event):
    ...
    if "image" in contentType:
        ...
        await gg.edit("🤗** Background Removed in `{}` seconds **\nPowered by @IMLALISAbot \nUpdates channel 👉 @sl_bot_zone ".format(ms))
    else:
        await gg.edit(
            "remove.bg API returned Errors. Please report to @CGSsupport\n`{}`\nor join 👉 @CGSsupport ".format(
             j   output_file_name.content.decode("UTF-8")
            )
        )
